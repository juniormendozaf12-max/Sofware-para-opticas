...
import { setEstablecimiento, STORES, type StoreConfig } from './lib/supabase';
import Layout from './components/Layout';
import Dashboard from './components/Dashboard';
import SplashScreen from './components/SplashScreen';

// Lazy-load heavy components — only loaded when their route is visited
...
export default function App() {
  ...
  const handleDoctorLogin = (storeId: string, doctor: typeof DOCTOR_PROFILES.dos_de_mayo[0]) => {
    setEstablecimiento(storeId);
    ...
    if (loading) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-white">
          <div className="w-8 h-8 rounded-full border-2 border-divider border-t-on-surface animate-spin" />
        </div>
      );
    }
...
// ══════════════════════════════════════════
// LOGIN SCREEN — Premium fluid design
// ══════════════════════════════════════════
...
function LoginScreen({
  ...
  onDoctorLogin: (storeId: string, doc: typeof DOCTOR_PROFILES.dos_de_mayo[0]) => void;
  onGoogleLogin: () => void;
  onBack: () => void;
}) {
  return (
